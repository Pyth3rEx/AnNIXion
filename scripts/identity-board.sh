...
icons=$(nix build --no-link --print-out-paths --impure --expr \
  'let pkgs = import <nixpkgs> {}; in import ./home/desktop/icons { inherit pkgs; lib = pkgs.lib; }')
...
python3 scripts/identity-board.py "$icons/share/icons/AnNIXion/scalable/apps" "$html"
